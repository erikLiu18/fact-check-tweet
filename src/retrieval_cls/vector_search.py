...
class FactCheckSearch:
    ...
    def extend_index(self, csv_path: str, batch_size: int = 32) -> None:
        ...
        Args:
            csv_path: Path to the additional CSV file
            batch_size: Batch size for generating embeddings
        """
        if self.index is None:
            raise ValueError("No existing index found. Use build_index() to create a new index first.")
            
        print(f"Loading additional data from {csv_path}...")
        df = pd.read_csv(csv_path)
        new_texts = df['text'].tolist()
        new_ratings = df['mergedTextualRating'].tolist()
        ...
        print(f"Index extended with {len(new_texts)} new documents. Total documents: {len(self.texts)}")
...
def main():
    ...
    parser.add_argument('--extend-index', action='store_true',
    ...
    if args.load_index or args.extend_index:
